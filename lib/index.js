...
function createClient(protoFile, directory, serviceName, address, options) {
  ...
  options.nestedPackages.forEach(function(pkg) {
    parsed[pkg] && Object.keys(parsed[pkg]).forEach(function(pkgElem){
      packages.push(pkg + "." + pkgElem);
    })
  })
...
function init(packageName, parsed, protoFile, serviceName, address, options) {
  ...
  if (options.token) {
    let extra_creds = grpc.credentials.createFromMetadataGenerator((args, callback) => {
  ...
  rs.context.metadata = new grpc.Metadata()
